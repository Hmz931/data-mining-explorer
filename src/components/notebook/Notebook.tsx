...
function highlightPython(src: string): string {
  ...
  return escape(src).split("\n").map((line) => {
    ...
    out = out.replace(BUILTINS, '<span class="text-sky-300">$1</span>');
    out = out.replace(KW, '<span class="text-violet-300 font-semibold">$1</span>');
    return out;
  }).join("\n");
}

export const NbCode = ({ code, language = "python" }: { code: string; language?: string }) => {
  const [copied, setCopied] = useState(false);
  const n = nextIn();
  const html = language === "python" ? highlightPython(code) : code.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
  return (
    <div className="grid grid-cols-[60px_1fr] bg-card group">
      <div className="pt-3 pr-2 text-right font-mono text-[11px] text-sky-700/70 select-none border-r border-border/40">
        In&nbsp;[{n}]:
      </div>
      <div className="relative bg-[hsl(220_30%_10%)]">
        ...
        <pre className="px-4 py-3 overflow-x-auto text-[12.5px] leading-[1.6] font-mono text-[hsl(42_30%_92%)]">
